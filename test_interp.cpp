...
int main() {
  ...
  timer::tic();
  for (int iter = 0; iter < max_iter; ++iter)
    value_interp[1] = image_processing::InterpolateImageIntensityWithPatchPattern(
  ...
  timer::tic();
  for (int iter = 0; iter < max_iter; ++iter)
    value_interp[2] = image_processing::InterpolateImageIntensityWithPatchSize(
      cv_image, patch_center_pixel, win_size, win_size);
  std::cout << "InterpolateImageIntensityWithPatchSize: " << timer::toc(0) << " [ms]\n";

  timer::tic();
  for (int iter = 0; iter < max_iter; ++iter)
    value_interp[3] = image_processing::InterpolateImageIntensityWithIntegerRow(
  ...
  // timer::tic();
  // for (int iter = 0; iter < max_iter; ++iter)
  //   image_processing::unsafe::InterpolateImageIntensitySameRatio(cv_image, pts_sample, shift,
  //   0.0,
  //                                                                value_interp[3]);
  ...
  // timer::tic();
  // for (int iter = 0; iter < max_iter; ++iter)
  //   image_processing::unsafe::InterpolateImageIntensitySameRatioHorizontal(cv_image, pts_sample,
  //                                                                          shift,
  //                                                                          value_interp[5]);
  // std::cout << "unsafe::InterpolateImageIntensitySameRatioHorizontal: " << timer::toc(0)
  //           << " [ms]\n";

  // timer::tic();
  // for (int iter = 0; iter < max_iter; ++iter)
  //   image_processing::unsafe::InterpolateImageIntensitySameRatioHorizontalRegularPattern(
  //     cv_image, cv::Point2f(u + shift, v), shift, win_size, value_interp[6]);
  // std::cout << "unsafe::InterpolateImageIntensitySameRatioHorizontalRegularPattern: "
  //           << timer::toc(0) << " [ms]\n";

  // timer::tic();
  // for (int iter = 0; iter < max_iter; ++iter)
  //   image_processing::unsafe::
  //     InterpolateImageIntensitySameRatioHorizontalRegularPatternArbitraryWindow(
  //       cv_image, cv::Point2f(u + shift, v), shift, 25, 25, 25, 25, value_interp[7]);
  // std::cout <<
  // "unsafe::InterpolateImageIntensitySameRatioHorizontalRegularPatternArbitraryWindow: "
  //           << timer::toc(0) << " [ms]\n";
